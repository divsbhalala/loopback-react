...
module.exports = function (User) {
  ...
  User.list = function (req, cb) {
    let {query, filter} = req.query;
    const {skip, limit} = filter || {skip: 0, limit: 10};
    const {body, params} = req;
    console.log({body, params, query, filter});
    query = query && query !== 'undefined' ? new RegExp(`^${query}`, 'i') : false;
    const where = query ? {
      and: [{status: true}, {
        or: [{name: query}
          , {surname: query}
    ...
    User.count(where,
      (err, count) => {
        User.find({
            where: where,
            //fields: {id: true, name: true, username: true, createdAt: true, status: true},
            order: 'name ASC',
            skip, limit
          },
          (err, items) => {
